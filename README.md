...
正式发布时两端同目录提供 `repository.json.sig` 以及索引引用的 `devices/`、`models/` 和 `canvases/` 文件。GitHub 是权威源，Gitee 只做相同提交的镜像；客户端以内置公钥验证签名，不因地址不同而建立不同信任根。当前开发提交已包含受保护发布工作流签署的索引与资源签名；资源变更推送后该工作流会重新签署。
